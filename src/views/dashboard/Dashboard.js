...
const Dashboard = (props) => {
  ...
  const getData = async () => {
    try {
      ...
      const response = await props.actions._getDashboardStats();
      console.log('========> get all dashboard repsonse', response);
    } catch (error) {

      console.log("Error in get dashboard stats", error.response);
...
function mapDispatchToProps(dispatch) {
  return {
      actions: bindActionCreators({
          _getDashboardStats: (data) => getDashboardStats(data),
      }, dispatch)
  }
}
...
function mapStateToProps(state) {
  ...
  const { subAdmins, dashboard } = state;
  return {
